{{-- Expense-voucher-style payment form for a single Bike Registration entry. --}}
@php
    $brHead = company_table('accounts')->where('id', ga_id('BIKE_REGISTRATION_EXPENSE_ACCOUNT'))->first();
    $rider = $rider ?? ($accounts->rider ?? (
        ($accounts->rider_id ?? $data->rider_id)
            ? company_table('riders')->where('id', $accounts->rider_id ?? $data->rider_id)->first()
            : null
    ));
    $defaultNarration = old('narration', $data->detail ?: ('Bike Registration Payment — ' . ($data->registration_status ?? '')));
...
<form enctype="multipart/form-data"
      action="{{ route('BikeRegistration.payfine') }}"
      method="POST"
      id="formajax"
      class="form-with-fixed-footer bike-registration-pay-form">
    @csrf
    ...
    <div class="card-body card-body-with-footer">
        <div class="row mt-0 mb-2" id="br-pay-voucher-header">
        ...
        <div class="alert alert-light border py-2 mb-3">
            <div class="row small g-2">
                <div class="col-md-4">
                    ...
                    <strong>
                        @if($rider)
                            {{ trim(($rider->rider_id ?? '') . ' — ' . ($rider->name ?? '')) }}
                        @else
                            {{ $accounts->name ?? '—' }}
                        @endif
                    </strong>
        ...
        <h6 class="mb-2">Bike Registration Entry</h6>
        ...
        <div class="row g-2 mb-2 align-items-start br-pay-expense-entry">
            <div class="col-md-3">
                <input type="text"
                       class="form-control"
                       value="{{ $brHead->name ?? 'Bike Registration Expense Account' }}"
                       readonly
                       tabindex="-1">
                <input type="hidden" name="expense_account_id" value="{{ ga_id('BIKE_REGISTRATION_EXPENSE_ACCOUNT') }}">
            </div>
            <div class="col-md-4">
    ...
    <div class="card-footer fixed-footer" style="z-index: 1 !important; text-align: right;">
        ...
        <button type="submit" class="btn btn-primary">
            <i class="fa fa-check me-1"></i> Pay Bike Registration
        </button>
    </div>
</form>
...
<script>
    (function() {
        function boot() {
            ...
            var $form = $('.bike-registration-pay-form').last();
            ...
            var $narration = $form.find('#br_pay_narration');
            function autoResize() {
            ...
            $narration.off('input.brPay').on('input.brPay', autoResize);
            autoResize();

            function calculateVat() {
                var amount = parseFloat($form.find('#br_pay_amount_display').val()) || 0;
                var vatPercent = parseFloat($form.find('#br_pay_vat_percent').val()) || 0;
                var vatAmount = Math.round((amount * vatPercent) / 100 * 100) / 100;
                $form.find('#br_pay_vat_amount').val(vatAmount > 0 ? vatAmount.toFixed(2) : '0.00');
            ...
            $form.find('#br_pay_vat_percent')
                .off('change.brPayVat keyup.brPayVat input.brPayVat')
                .on('change.brPayVat keyup.brPayVat input.brPayVat', calculateVat);
