            ...
            <div class="form-group col-md-3">
                ...
                <select name="bike_id" class="form-control select2 bike" id="bike_select">
                    ...
                    <option value="{{ $bike->id }}"
                        data-rider="{{ $bike->rider ? $bike->rider->rider_id .'-'. $bike->rider->name : '' }}"
                        data-rider-id="{{ $bike->rider->id ?? '' }}"
                        data-previous-km="{{ $bike->previous_km ?? '' }}"
                        data-maintenance-km="{{ $bike->maintenance_km ?? '' }}"
                        data-current-km="{{ $bike->current_km ?? '' }}">
    ...
    <div class="d-flex justify-content-between align-items-center gap-3">
        ...
        <div class="d-flex align-items-center">
            <div class="input-group flex-nowrap">
                ...
                {{-- Changed ID to maintenance_total_cost to avoid conflict --}}
...
$(document).ready(function() {
    ...
    function calculateOverdue() {
        ...
        const manitenance = parseFloat(maintenanceAt.val());
    ...
    $(document).on('change', '#bike_select', function(){
        ...
        const currentKmData = selectedOption.data('current-km');
        ...
        if(riderData) {
            ...
            riderInfo.val("No Rider Assigned");
        }
        if(riderId) {
            riderIdHidden.val(riderId);
        } else {
            riderIdHidden.val(null);
        }
        if(previousKmData) {
            previousKm.val(previousKmData);
            previousKm.prop('readonly', true);
        } else {
            previousKm.val('');
            previousKm.prop('readonly', false);
        }
        if(currentKmData){
            currentKm.val(currentKmData)
        }else{
            currentKm.val('')
        }
        if(maintenanceKmData) {
