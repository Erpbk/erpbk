...
{
    /**
     * Tables that are central/system scoped and should not be tenant-scoped by company_id.
     *
    ...
    public function up(): void
    {
        ...
        foreach ($this->tenantTables($database) as $table) {
            ...
            // Legacy imports may contain zero dates (e.g. 0000-00-00). MySQL can
            // reject unrelated ALTER TABLE operations in strict mode until those are cleaned.
            $this->normalizeZeroDates($database, $table);

            // Backfill existing rows so old data is immediately usable in company_id mode.
