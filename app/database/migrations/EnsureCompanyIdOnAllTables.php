...
/**
 * Idempotently adds company_id (+ index, backfill, FK) to every tenant-scoped table.
 */
class EnsureCompanyIdOnAllTables extends Migration
{
    /**
     * Central/system tables that must not receive company_id.
     *
     * @var list<string>
     */
     ...
     * @return list<string>
     */
    protected function tenantTables(string $database): array
    {
        $rows = DB::select(
            "SELECT table_name
    ...
    protected function hasIndex(string $database, string $table, string $column): bool
    {
        $row = DB::selectOne(
            "SELECT COUNT(*) AS c
    ...
    protected function hasCompanyForeignKey(string $database, string $table): bool
    {
        $row = DB::selectOne(
            "SELECT COUNT(*) AS c
     ...
     * @return list<string>
     */
    protected function foreignKeysForCompanyId(string $database, string $table): array
    {
        $rows = DB::select(
     ...
     * @return list<string>
     */
    protected function indexesForCompanyId(string $database, string $table): array
    {
        $rows = DB::select(
    ...
    protected function makeName(string $base): string
    {
        if (strlen($base) <= 64) {
            return $base;
        }
    ...
    protected function normalizeOrphanedCompanyIds(string $table, int $fallbackCompanyId): void
    {
    ...
    protected function normalizeZeroDates(string $database, string $table): void
    {
        $columns = DB::select(
    ...
    protected function resolveFallbackCompanyId(string $database): int
    {
