...
$licenseStatusReturnTo = $licenseStatusReturnTo ?? ($selectedCategoryId ? (route($licenseRoute . '.index') . '?category_id=' . $selectedCategoryId) : null);
@endphp
<style>
    .visa-row-actions {
        display: inline-flex;
...
<div class="table-responsive">
    <table class="table table-striped" id="license-statuses-table">
        ...
        <tbody id="license-statuses-tbody">
            ...
            <tr data-id="{{ $status->id }}" data-category-id="{{ (int) $status->license_category_id }}">
                ...
                <td>
                    <div class="visa-row-actions">
                        ...
                        <button
                            ...
                            data-bs-target="#editLicenseExpenseStatusModal"
                            data-id="{{ $status->id }}"
                            data-code="{{ $status->code }}"
                            data-name="{{ $status->name }}"
                            data-category="{{ $status->category }}"
                            data-license-category-id="{{ (int) $status->license_category_id }}"
                        ...
                        @else
                        <a href="{{ route($licenseRoute . '.edit', $status->id) }}" class="visa-icon-btn visa-icon-btn-edit" title="Edit">
                            <i class="ti ti-pencil"></i>
                        </a>
                        @endif
                        <a href="{{ route($licenseRoute . '.toggle-active', $status->id) . ($licenseStatusReturnTo ? ('?return_to=' . urlencode($licenseStatusReturnTo)) : '') }}"
            ...
            <tr>
                <td colspan="9" class="text-center text-muted py-5">
                    <div class="py-3">
                        ...
                        <div>No license statuses in this category.</div>
                        <div class="small">Add statuses here before generating license expense tickets for this category.</div>
                    </div>
                </td>
            </tr>
            @endforelse
