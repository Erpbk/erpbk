<table class="table table-striped dataTable no-footer" id="dataTableBuilder">
   <thead class="text-center">
      <tr role="row">
         <th title="Transaction ID" class="sorting" tabindex="0" rowspan="1" colspan="1">Transaction ID</th>
         <th title="Rider Name" class="sorting" tabindex="0" rowspan="1" colspan="1">Rider Name</th>
         <th title="Billing Month" class="sorting" tabindex="0" rowspan="1" colspan="1">Billing Month</th>
         <th title="Trip Date" class="sorting" tabindex="0" rowspan="1" colspan="1">Trip Date</th>
         <th title="Trip Time" class="sorting" tabindex="0" rowspan="1" colspan="1">Trip Time</th>
         <th title="Toll Gate" class="sorting" tabindex="0" rowspan="1" colspan="1">Toll Gate</th>
         <th title="Direction" class="sorting" tabindex="0" rowspan="1" colspan="1">Direction</th>
         <th title="Tag Number" class="sorting" tabindex="0" rowspan="1" colspan="1">Tag Number</th>
         <th title="Plate No" class="sorting" tabindex="0" rowspan="1" colspan="1">Plate No</th>
         <th title="Amount" class="sorting" tabindex="0" rowspan="1" colspan="1">Total Amount</th>
   ...
   <tbody>
      ...
      <tr class="text-center">
         ...
         <td>
            @if($rider)
            <a href="{{ route('riders.show', $rider->id) }}">{{ $rider->rider_id }} - {{ $rider->name }}</a>
            @else
            -
            @endif
         </td>
         ...
         <td style="position: relative;">
            <div class="dropdown">
               ...
               <div class="dropdown-menu dropdown-menu-end" aria-labelledby="actiondropdown_{{ $item['id'] }}" style="z-index: 1050;">
                  ...
                  <form id="restore-form-{{ $item['module'] }}-{{ $item['id'] }}" action="{{ route(($trashRoute ?? 'settings-panel.trash') . '.restore', ['module' => $item['module'], 'id' => $item['id']]) }}" method="POST" style="display: none;">
                     @csrf
                  </form>
                  @endif
                  ...
                  <form id="delete-form-{{ $item['module'] }}-{{ $item['id'] }}" action="{{ route(($trashRoute ?? 'settings-panel.trash') . '.force-destroy', ['module' => $item['module'], 'id' => $item['id']]) }}" method="POST" style="display: none;">
                     @csrf
                     @method('DELETE')
                  </form>
                  @endif
      ...
      <tr>
         <td colspan="12" class="text-center">
            <div class="py-4">
               ...
               <p class="text-muted mb-0">No deleted salik records found</p>
            </div>
         </td>
      </tr>
      @endif
   ...
   <nav>
      <ul class="pagination justify-content-center mb-0">
         <li class="page-item {{ $currentPage == 1 ? 'disabled' : '' }}">
            <a class="page-link" href="{{ route(($trashRoute ?? 'settings-panel.trash') . '.index', array_merge(request()->all(), ['page' => $currentPage - 1])) }}">
               Previous
            </a>
         </li>
         ...
         @for($i = 1; $i <= $totalPages; $i++)
            ...
            <a class="page-link" href="{{ route(($trashRoute ?? 'settings-panel.trash') . '.index', array_merge(request()->all(), ['page' => $i])) }}">
               {{ $i }}
            </a>
            </li>
            @endfor
