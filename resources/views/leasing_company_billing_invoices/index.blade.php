                    ...
                    <div class="action-buttons d-flex justify-content-end">
                        <div class="action-dropdown-container">
                            ...
                            <div class="action-dropdown-menu" id="addBikeDropdown">
                                <a class="action-dropdown-item show-modal" href="javascript:void(0);" data-size="xl" data-title="Add New Leasing Company Billing Invoice" data-action="{{ route('leasingCompanyBillingInvoices.create') }}">
                                    <i class="ti ti-plus"></i>
                                    <div>
    ...
    <div class="filter-body" id="searchTopbody">
        <form id="filterForm" action="{{ route('leasingCompanyInvoices.index') }}" method="GET">
            <div class="row">
                ...
                <div class="form-group col-md-12">
                    <label for="billing_month">Status</label>
                    <select class="form-control" id="status" name="status">
                        <option value="">Select</option>
                        <option value="1" {{ request('status') == 1 ? 'selected' : '' }}>Paid</option>
                        <option value="0" {{ request('status') == 3 ? 'selected' : '' }}>Partially Paid</option>
...
<div class="content mt-3">
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive px-2 py-0" id="table-data">
            @include('leasing_company_billing_invoices.table')
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#filterForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('leasingCompanyBillingInvoices.index') }}",
                type: "GET",
                data: formData,
                success: function(data) {
                    $('#table-data').html(data.tableData);
                    let newUrl = "{{ route('leasingCompanyBillingInvoices.index') }}" + (formData ? '?' + formData : '');
