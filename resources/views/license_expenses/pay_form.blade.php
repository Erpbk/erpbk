{{-- Expense-voucher-style payment form for a single License Expense entry. --}}
@php
    $licenseHead = company_table('accounts')->where('id', ga_id('LICENSE_EXPENSE_ACCOUNT'))->first();
    $rider = $rider ?? ($accounts->rider ?? company_table('riders')->where('id', $accounts->rider_id ?? $data->rider_id)->first());
    $defaultNarration = old('narration', $data->detail ?: ('License Expense Payment — ' . ($data->license_status ?? '')));
...
<form enctype="multipart/form-data"
      action="{{ route('LicenseExpense.payfine') }}"
      method="POST"
      id="formajax"
      class="form-with-fixed-footer license-expense-pay-form">
    @csrf
    ...
    <input type="hidden" name="amount" id="license_pay_amount" value="{{ number_format($baseAmount, 2, '.', '') }}">
    <input type="hidden" name="Created_By" value="{{ Auth::user()->id }}">
    <input type="hidden" name="payment_type" id="license_pay_payment_type" value="Asset">

    <div class="card-body card-body-with-footer">
        <div class="row mt-0 mb-2" id="license-pay-voucher-header">
        ...
        <div class="row mb-2">
            <div class="form-group col-md-4">
                <label for="license_pay_attach_file">Document</label>
                <input type="file"
                       name="attach_file"
                       id="license_pay_attach_file"
                       class="form-control"
        ...
        <div class="alert alert-light border py-2 mb-3">
            <div class="row small g-2">
                ...
                <div class="col-md-4">
                    <span class="text-muted">License Status:</span>
                    <strong>{{ $data->license_status ?? '—' }}</strong>
                </div>
                <div class="col-md-4">
        ...
        <h6 class="mb-2">License Expense Entry</h6>
        ...
        <div class="row g-2 mb-2 align-items-start license-pay-expense-entry">
            <div class="col-md-3">
                <input type="text"
                       class="form-control"
                       value="{{ $licenseHead->name ?? 'License Expense Account' }}"
                       readonly
                       tabindex="-1">
                <input type="hidden" name="expense_account_id" value="{{ ga_id('LICENSE_EXPENSE_ACCOUNT') }}">
            </div>
            <div class="col-md-4">
                <textarea name="narration"
                          id="license_pay_narration"
    ...
    <div class="card-footer fixed-footer" style="z-index: 1 !important; text-align: right;">
        ...
        <button type="submit" class="btn btn-primary">
            <i class="fa fa-check me-1"></i> Pay License Expense
        </button>
    </div>
</form>
...
<script>
    (function() {
        function boot() {
            ...
            var $form = $('.license-expense-pay-form').last();
            ...
            var $narration = $form.find('#license_pay_narration');
            function autoResize() {
            ...
            $narration.off('input.licensePay').on('input.licensePay', autoResize);
            autoResize();

            function calculateVat() {
                var amount = parseFloat($form.find('#license_pay_amount_display').val()) || 0;
                var vatPercent = parseFloat($form.find('#license_pay_vat_percent').val()) || 0;
                var vatAmount = Math.round((amount * vatPercent) / 100 * 100) / 100;
                $form.find('#license_pay_vat_amount').val(vatAmount > 0 ? vatAmount.toFixed(2) : '0.00');
            ...
            $form.find('#license_pay_vat_percent')
                .off('change.licensePayVat keyup.licensePayVat input.licensePayVat')
                .on('change.licensePayVat keyup.licensePayVat input.licensePayVat', calculateVat);
