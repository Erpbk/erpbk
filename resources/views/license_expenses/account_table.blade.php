...
<table class="table table-striped dataTable no-footer" id="dataTableBuilder">
   ...
   <tbody>
      ...
      $categoryId = (int) ($r->license_category_id ?? \App\Support\LicenseCategoryService::defaultCategory()->id);
      $balance = \App\Support\LicenseCategoryService::expensesForAccountQuery(
      (int) $r->id,
      ...
      <tr class="text-center">
         ...
         <td class="text-start"><a href="{{ route('LicenseExpense.generatentries' , $r->id) }}">{{ $r->name }}</a></td>
         <td>{{ $r->licenseCategory->name ?? '—' }}</td>
         ...
         <td class="align-middle @if($nextUnpaid) visa-next-unpaid-cell @endif">
            @if($nextUnpaid)
            <a href="{{ route('LicenseExpense.generatentries', $r->id) }}" class="text-decoration-none text-body visa-next-unpaid-blink d-inline-block text-center">
               <span class="fw-semibold d-block text-body text-center">{{ $nextUnpaid->license_status ?? '—' }}</span>
               @if($nextWhen !== '')
         ...
         <td class="align-middle @if($urgentExpiry && $urgentExpiryWhen !== '') visa-expiry-alert-cell @endif">
            ...
            <a href="{{ route('LicenseExpense.generatentries', $r->id) }}" class="text-decoration-none text-body visa-expiry-alert-blink d-inline-block text-center" title="Visa document expiry within 10 days or overdue">
               <span class="fw-semibold d-block text-body text-center">{{ $urgentExpiry->license_status ?? '—' }}</span>
         ...
         <div class="modal-dialog modal-lg">
            <div class="modal-content">
               ...
               <div class="modal-body">
                  <form action="{{ route('LicenseExpense.editaccount') }}" method="POST">
                     @csrf
