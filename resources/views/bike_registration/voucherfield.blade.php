...
<div class="form-group col-sm-6">
    ...
    <select class="form select select2" id="registration_status" name="registration_status" readonly>
        <option value=""></option>
        @foreach ($registrationStatusesList as $status)
        <option value="{{ $status->name }}" @if($data->registration_status == $status->name) selected @endif>{{ $status->name }}</option>
        @endforeach
    </select>
</div>
<div class="form-group col-sm-6">
