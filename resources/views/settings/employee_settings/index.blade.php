...
<style>
  ...
  .employee-top-visibility-controls {
    background: #f8f9fb;
  ...
  .employee-top-visibility-controls .form-check {
    min-height: auto;
    margin-bottom: 0;
    display: inline-flex;
    align-items: center;
  ...
  .employee-top-visibility-controls .form-check-input {
    width: 2rem;
    height: 1.1rem;
    margin: 0;
    cursor: pointer;
  ...
  .employee-top-visibility-controls .form-check-label {
    font-size: 0.78rem;
    font-weight: 500;
    color: #5f6b7a;
    margin-top: 0;
      ...
      <div class="card-body">
        ...
        <div class="tab-content" id="employeeSettingsTabContent">
          ...
          <div class="tab-pane fade show active" id="tab-general" role="tabpanel">
            ...
            <form action="{{ route('settings-panel.employee-settings.store-module-label') }}" method="POST" class="row g-3 align-items-end">
              @csrf
              <div class="col-md-6">
                <label class="form-label">Name in menu</label>
                <input type="text" name="module_label" class="form-control" value="{{ old('module_label', $moduleLabel ?? 'Employee Settings') }}" placeholder="Employee Settings" maxlength="100" required>
              </div>
              <div class="col-md-6">
          ...
          <div class="tab-pane fade" id="tab-employee-top" role="tabpanel">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
              ...
              <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addEmployeeTopCategoryModal">
                <i class="ti ti-plus me-1"></i> Add Category
              </button>
            </div>
            <div id="employeeTopAccordionContainer">
...
{{-- Add Employee Top Category modal --}}
<div class="modal fade" id="addEmployeeTopCategoryModal" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title">Add Employee Top Category</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="formAddEmployeeTopCategory">
        @csrf
        <div class="modal-body pt-0">
          <div class="mb-3">
            <label class="form-label">Rider Dropdown Column <span class="text-danger">*</span></label>
            <select name="employee_column" id="addEmployeeTopCategoryColumn" class="form-select select2" data-placeholder="Select dropdown column" required>
              <option value="">Select dropdown column</option>
              @foreach(($employeeTopSelectableColumns ?? []) as $columnKey => $columnLabel)
              <option value="{{ $columnKey }}">{{ $columnLabel }} ({{ $columnKey }})</option>
              @endforeach
            </select>
            <div class="form-text">Only employee dropdown columns are available here; foreign-key and externally linked columns are excluded.</div>
          </div>
        </div>
        <div class="modal-footer border-0 pt-0">
          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary" id="addEmployeeTopCategorySubmitBtn">Save</button>
        </div>
      </form>
    </div>
  </div>
...
{{-- Add Employee Top Option modal --}}
<div class="modal fade" id="addEmployeeTopOptionModal" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title">Add Employee Top Option</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="formAddEmployeeTopOption">
        @csrf
        <input type="hidden" name="category_id" id="addEmployeeTopOptionCategoryId">
        <div class="modal-body pt-0">
          <div class="mb-2 text-muted small">
            Category: <strong id="addEmployeeTopOptionCategoryName">-</strong>
          </div>
          <div class="mb-2 text-muted small">
            Source Column: <strong id="addEmployeeTopOptionColumnName">-</strong>
          </div>
          <div class="mb-3">
            <label class="form-label d-block">Selection Mode</label>
            <div class="d-flex gap-3">
              <div class="form-check">
                <input class="form-check-input" type="radio" name="selection_mode" id="employeeTopOptionModeSingle" value="single" checked>
                <label class="form-check-label" for="employeeTopOptionModeSingle">Single Select</label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="radio" name="selection_mode" id="employeeTopOptionModeMultiple" value="multiple">
                <label class="form-check-label" for="employeeTopOptionModeMultiple">Multiple Select</label>
              </div>
            </div>
          </div>
          <div class="mb-3">
            <label class="form-label">Add Option Value(s) <span class="text-danger">*</span></label>
            <div id="addEmployeeTopOptionRows" class="d-flex flex-column gap-2"></div>
            <button type="button" class="btn btn-sm btn-outline-primary mt-2" id="addEmployeeTopOptionRowBtn">Add Option</button>
            <div class="form-text">Values are loaded from the selected category column in the employee table. You can add one or more items.</div>
          </div>
        </div>
        <div class="modal-footer border-0 pt-0">
          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary" id="addEmployeeTopOptionSubmitBtn">Save</button>
        </div>
      </form>
    </div>
  </div>
...
{{-- Edit Employee Top Category modal --}}
<div class="modal fade" id="editEmployeeTopCategoryModal" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title">Edit Employee Top Category</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="formEditEmployeeTopCategory">
        @csrf
        <input type="hidden" name="id" id="editEmployeeTopCategoryId">
        <div class="modal-body pt-0">
          <div class="mb-3">
            <label class="form-label">Category Name <span class="text-danger">*</span></label>
            <input type="text" name="name" id="editEmployeeTopCategoryName" class="form-control" maxlength="255" required>
          </div>
        </div>
        <div class="modal-footer border-0 pt-0">
          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary" id="editEmployeeTopCategorySubmitBtn">Update</button>
        </div>
      </form>
    </div>
  </div>
...
{{-- Edit Employee Top Option modal --}}
<div class="modal fade" id="editEmployeeTopOptionModal" tabindex="-1" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title">Edit Employee Top Option</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="formEditEmployeeTopOption">
        @csrf
        <input type="hidden" name="id" id="editEmployeeTopOptionId">
        <div class="modal-body pt-0">
          <div class="mb-3">
            <label class="form-label">Option Name <span class="text-danger">*</span></label>
            <input type="text" name="name" id="editEmployeeTopOptionName" class="form-control" maxlength="255" required>
          </div>
        </div>
        <div class="modal-footer border-0 pt-0">
          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary" id="editEmployeeTopOptionSubmitBtn">Update</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  (function() {
    ...
    window.refreshEmployeeCustomFieldsCategory = function(categoryId) {
      ...
      if (!tbody) return;
      var url = "{{ url('settings-panel/employee-settings/fields/table-body') }}/" + categoryId;
      fetch(url, {
          headers: {
    ...
    window.refreshRiderTopAccordion = function() {
      fetch("{{ route('settings-panel.employee-settings.employee-top-accordion-body') }}", {
          headers: {
            'X-Requested-With': 'XMLHttpRequest'
          }
        })
        ...
        .then(function(html) {
          var container = document.getElementById('employeeTopAccordionContainer');
    ...
    // Select2 inside Employee Top modal (with correct dropdown parent).
    function initTopCategoryColumnSelect2() {
      if (!(window.jQuery && jQuery.fn && jQuery.fn.select2)) return;
      var $topCategoryColumn = jQuery('#addEmployeeTopCategoryColumn');
      var $modal = jQuery('#addEmployeeTopCategoryModal');
    ...
    if (window.jQuery) {
      jQuery('#addEmployeeTopCategoryModal').on('shown.bs.modal', function() {
        initTopCategoryColumnSelect2();
      });
    }

    var employeeTopAvailableValues = [];

    function createEmployeeTopOptionRow(initialValue) {
      var rowsWrap = document.getElementById('addEmployeeTopOptionRows');
      if (!rowsWrap) return;
      var row = document.createElement('div');
      row.className = 'd-flex align-items-center gap-2';

      var input = document.createElement('select');
      input.className = 'form-select employee-top-option-row-input';
      input.appendChild(new Option('Select value', ''));
      employeeTopAvailableValues.forEach(function(v) {
    ...
    function setRiderTopSelectionMode(mode) {
      var addBtn = document.getElementById('addEmployeeTopOptionRowBtn');
      var rowsWrap = document.getElementById('addEmployeeTopOptionRows');
      if (!rowsWrap) return;
      var rows = rowsWrap.querySelectorAll('.employee-top-option-row-input');
    ...
    function setEmployeeTopOptionSuggestions(values) {
      employeeTopAvailableValues = Array.isArray(values) ? values : [];
      var rowsWrap = document.getElementById('addEmployeeTopOptionRows');
      if (!rowsWrap) return;
      rowsWrap.querySelectorAll('.employee-top-option-row-input').forEach(function(selectEl) {
        ...
        if (currentValue && employeeTopAvailableValues.indexOf(currentValue) !== -1) {
          selectEl.value = currentValue;
        }
      });
    }

    var formAddCat = document.getElementById('formAddEmployeeCategory');
    if (formAddCat) {
      formAddCat.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var btn = document.getElementById('addEmployeeCategorySubmitBtn');
        var fd = new FormData(form);
        if (btn) btn.disabled = true;
        fetch(form.action || "{{ route('settings-panel.employee-settings.store-category') }}", {
            method: 'POST',
            body: fd,
            headers: {
              'X-CSRF-TOKEN': csrf,
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('addEmployeeCategoryModal'));
                if (modal) modal.hide();
              }
              form.reset();
    ...
    var formAddEmployeeTopCategory = document.getElementById('formAddEmployeeTopCategory');
    if (formAddEmployeeTopCategory) {
      formAddEmployeeTopCategory.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var btn = document.getElementById('addEmployeeTopCategorySubmitBtn');
        if (btn) btn.disabled = true;
        fetch("{{ route('settings-panel.employee-settings.store-employee-top-category') }}", {
            method: 'POST',
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('addEmployeeTopCategoryModal'));
                if (modal) modal.hide();
              }
              form.reset();
    ...
    document.addEventListener('click', function(e) {
      var addOptionBtn = e.target.closest('.btn-add-employee-top-option');
      if (!addOptionBtn) return;
      var categoryIdInput = document.getElementById('addEmployeeTopOptionCategoryId');
      var categoryNameEl = document.getElementById('addEmployeeTopOptionCategoryName');
      var columnNameEl = document.getElementById('addEmployeeTopOptionColumnName');
      var rowsWrap = document.getElementById('addEmployeeTopOptionRows');
      var singleModeInput = document.getElementById('employeeTopOptionModeSingle');
      ...
      if (rowsWrap) {
        rowsWrap.innerHTML = '';
        createEmployeeTopOptionRow('');
      }
      setRiderTopSelectionMode('single');

      var categoryId = addOptionBtn.getAttribute('data-category-id') || '';
      if (!categoryId) return;
      const fieldValuesUrlTemplate = "{{ route('settings-panel.employee-settings.employee-top-category-field-values', ['id' => '__CID__']) }}";
      ...
      fetch(fieldValuesUrl, {
        ...
        .then(function(data) {
          if (!data.success) {
            setEmployeeTopOptionSuggestions([]);
            if (columnNameEl) columnNameEl.textContent = '-';
            return;
          }
          if (columnNameEl) columnNameEl.textContent = data.column || '-';
          var values = Array.isArray(data.values) ? data.values : [];
          setEmployeeTopOptionSuggestions(values);
          if (values.length && rowsWrap) {
            var firstInput = rowsWrap.querySelector('.employee-top-option-row-input');
    ...
    var addEmployeeTopOptionRowBtn = document.getElementById('addEmployeeTopOptionRowBtn');
    if (addEmployeeTopOptionRowBtn) {
      addEmployeeTopOptionRowBtn.addEventListener('click', function() {
        createEmployeeTopOptionRow('');
      });
    }
    ...
    document.addEventListener('click', function(e) {
      if (
        e.target.closest('.employee-top-visibility-controls') ||
        e.target.closest('.btn-edit-employee-top-category') ||
        e.target.closest('.btn-delete-employee-top-category') ||
        e.target.closest('.btn-edit-employee-top-option') ||
        e.target.closest('.btn-delete-employee-top-option')
      ) {
        e.stopPropagation();
      }
    });
    ...
    document.addEventListener('click', function(e) {
      var editCategoryBtn = e.target.closest('.btn-edit-employee-top-category');
      if (!editCategoryBtn) return;
      var idInput = document.getElementById('editEmployeeTopCategoryId');
      var nameInput = document.getElementById('editEmployeeTopCategoryName');
      ...
      if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
        var modal = new bootstrap.Modal(document.getElementById('editEmployeeTopCategoryModal'));
        modal.show();
      }
    });

    document.addEventListener('click', function(e) {
      var editOptionBtn = e.target.closest('.btn-edit-employee-top-option');
      if (!editOptionBtn) return;
      var idInput = document.getElementById('editEmployeeTopOptionId');
      var nameInput = document.getElementById('editEmployeeTopOptionName');
      ...
      if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
        var modal = new bootstrap.Modal(document.getElementById('editEmployeeTopOptionModal'));
        modal.show();
      }
    });

    document.addEventListener('click', function(e) {
      var deleteCategoryBtn = e.target.closest('.btn-delete-employee-top-category');
      ...
      var doDelete = function() {
        ...
        const deleteUrlTemplate = "{{ route('settings-panel.employee-settings.destroy-employee-top-category', ['id' => '__CID__']) }}";
    ...
    document.addEventListener('click', function(e) {
      var deleteOptionBtn = e.target.closest('.btn-delete-employee-top-option');
      ...
      var doDelete = function() {
        var fd = new FormData();
        fd.append('_method', 'DELETE');
        const deleteUrlTemplate = "{{ route('settings-panel.employee-settings.destroy-employee-top-option', ['id' => '__OID__']) }}";
    ...
    document.addEventListener('change', function(e) {
      var toggle = e.target.closest('.employee-top-visibility-toggle');
      if (!toggle) return;

      var controls = toggle.closest('.employee-top-visibility-controls');
      var categoryId = controls ? controls.getAttribute('data-category-id') : null;
      if (!categoryId) return;

      var topBarToggle = controls.querySelector('.employee-top-visibility-toggle[data-field="show_in_top_bar"]');
      var viewCardsToggle = controls.querySelector('.employee-top-visibility-toggle[data-field="show_in_view_cards"]');
      ...
      const updateVisibilityUrlTemplate = "{{ route('settings-panel.employee-settings.update-employee-top-category-visibility', ['id' => '__CID__']) }}";
    ...
    var formAddEmployeeTopOption = document.getElementById('formAddEmployeeTopOption');
    if (formAddEmployeeTopOption) {
      formAddEmployeeTopOption.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var btn = document.getElementById('addEmployeeTopOptionSubmitBtn');
        var rowsWrap = document.getElementById('addEmployeeTopOptionRows');
        ...
        var items = [];
        if (rowsWrap) {
          items = Array.prototype.slice.call(rowsWrap.querySelectorAll('.employee-top-option-row-input'))
            .map(function(el) {
              return (el.value || '').trim();
            })
            .filter(function(v) {
        ...
        fetch("{{ route('settings-panel.employee-settings.store-employee-top-option') }}", {
            method: 'POST',
            body: payload,
            headers: {
              'X-CSRF-TOKEN': csrf,
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('addEmployeeTopOptionModal'));
                if (modal) modal.hide();
              }
              form.reset();
              window.refreshRiderTopAccordion();
    ...
    var formEditEmployeeTopCategory = document.getElementById('formEditEmployeeTopCategory');
    if (formEditEmployeeTopCategory) {
      formEditEmployeeTopCategory.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var id = document.getElementById('editEmployeeTopCategoryId') && document.getElementById('editEmployeeTopCategoryId').value;
        var btn = document.getElementById('editEmployeeTopCategorySubmitBtn');
        if (!id) return;
        if (btn) btn.disabled = true;

        var fd = new FormData(form);
        fd.append('_method', 'PUT');
        const updateUrlTemplate = "{{ route('settings-panel.employee-settings.update-employee-top-category', ['id' => '__CID__']) }}";
        ...
        fetch(updateUrl, {
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('editEmployeeTopCategoryModal'));
                if (modal) modal.hide();
              }
              window.refreshRiderTopAccordion();
              if (typeof Swal !== 'undefined') Swal.fire({
    ...
    var formEditEmployeeTopOption = document.getElementById('formEditEmployeeTopOption');
    if (formEditEmployeeTopOption) {
      formEditEmployeeTopOption.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var id = document.getElementById('editEmployeeTopOptionId') && document.getElementById('editEmployeeTopOptionId').value;
        var btn = document.getElementById('editEmployeeTopOptionSubmitBtn');
        if (!id) return;
        if (btn) btn.disabled = true;

        var fd = new FormData(form);
        fd.append('_method', 'PUT');
        const updateUrlTemplate = "{{ route('settings-panel.employee-settings.update-employee-top-option', ['id' => '__OID__']) }}";
        ...
        fetch(updateUrl, {
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('editEmployeeTopOptionModal'));
                if (modal) modal.hide();
              }
              window.refreshRiderTopAccordion();
              if (typeof Swal !== 'undefined') Swal.fire({
