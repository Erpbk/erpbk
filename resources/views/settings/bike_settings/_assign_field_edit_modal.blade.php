<div class="modal fade" id="editBikeAssignFieldModal" tabindex="-1" data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title" id="editBikeAssignFieldModalTitle">Edit assign field</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <form id="formEditBikeAssignField" action="#" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" id="editBikeAssignFieldSubmitBtn">Save</button>
        </div>
      </form>
    </div>
  </div>
...
(function() {
  ...
  window.bikeToggleAssignBuiltinOptions = toggleAssignBuiltinOptionsWrap;
  window.bikeToggleAssignCustomOptions = toggleAssignCustomOptionsWrap;
  ...
  function renderOptions(rowsEl, hiddenEl, rawOptions) {
    if (typeof bikeRenderOptionRows === 'function') {
      bikeRenderOptionRows(rowsEl, hiddenEl, rawOptions || '');
      return;
    }
    ...
    lines.forEach(function(line) {
      if (typeof bikeCreateOptionRow === 'function') {
        bikeCreateOptionRow(rowsEl, hiddenEl, line);
      }
    });
  }

  function populateAssignEditModal(triggerEl) {
    var btn = triggerEl && triggerEl.closest ? triggerEl.closest('.btn-edit-bike-assign-field') : null;
    if (!btn) return;

    var payload = parsePayload(btn.getAttribute('data-assign-payload'));
    if (!payload) return;

    var form = document.getElementById('formEditBikeAssignField');
    ...
    var titleEl = document.getElementById('editBikeAssignFieldModalTitle');
    var submitBtn = document.getElementById('editBikeAssignFieldSubmitBtn');
  ...
  var editModalEl = document.getElementById('editBikeAssignFieldModal');
  if (editModalEl) {
  ...
  var editForm = document.getElementById('formEditBikeAssignField');
  if (editForm) {
    editForm.addEventListener('submit', function() {
      ...
      if (customVisible) {
        if (typeof bikeSyncOptionsToHidden === 'function') {
          bikeSyncOptionsToHidden(
            document.getElementById('editAssignCustomOptionsRows'),
            document.getElementById('editAssignCustomConfigOptionsHidden')
          );
        }
      } else if (typeof bikeSyncOptionsToHidden === 'function') {
        bikeSyncOptionsToHidden(
          document.getElementById('editAssignBuiltinOptionsRows'),
          document.getElementById('editAssignBuiltinConfigOptionsHidden')
        );
