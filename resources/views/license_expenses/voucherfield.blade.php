...
<div class="form-group col-sm-6">
    ...
    <select class="form select select2" id="license_status" name="license_status" readonly>
        <option value=""></option>
        @foreach ($licenseStatuses as $status)
        <option value="{{ $status->name }}" @if($data->license_status == $status->name) selected @endif>{{ $status->name }}</option>
        @endforeach
    </select>
</div>
<div class="form-group col-sm-6">
...
<div class="form-group col-sm-6">
    ...
    <select class="form-control select select2" id="expense_account_id" name="expense_account_id" readonly>
        <option value=""></option>
        <option value="{{ \App\Helpers\HeadAccount::LICENSE_EXPENSE_ACCOUNT }}" selected>{{ company_table('accounts')->where('id', \App\Helpers\HeadAccount::LICENSE_EXPENSE_ACCOUNT)->first()->name }}</option>
    </select>
</div>
<div class="form-group col-sm-6">
