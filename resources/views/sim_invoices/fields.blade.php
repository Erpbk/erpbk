...
<div class="row">
    ...
    <div class="col-md-4 form-group">
        <label>Vendor</label>
        @php
        $selectedVendor = isset($cloneFromInvoice) ? $cloneFromInvoice->vendor_id : (isset($invoice) ? $invoice->vendor_id : (isset($vendor) && $vendor ? $vendor->id : null));
        $isClone = isset($cloneFromInvoice);
        @endphp
        {!! Form::select('vendor_id', $vendors, $selectedVendor, ['class' => 'form-select form-select-sm select2', 'id' => 'vendor_id', 'disabled' => $isClone]) !!}
        @if($isClone)
...
$(document).ready(function() {
    ...
    if ($.fn.select2) {
        $('#vendor_id, .sim-select').select2({ dropdownParent: $modalBody.length ? $modalBody : $('body'), width: '100%' });
    }
    ...
    $('#add-new-row').off('click').on('click', function() {
        ...
        if ($.fn.select2) {
            $('#rows-container .invoice-item-row:last .sim-select').select2({ dropdownParent: $modalBody.length ? $modalBody : $('body'), width: '100%' });
        }
    });
