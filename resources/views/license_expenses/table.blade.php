...
<div id="visa-expenses-inline-edit-scope">
            ...
            <td>
               <span id="voucher_ids_display_{{ $r->id }}" class="d-inline-flex flex-wrap align-items-center justify-content-center gap-1">
                  ...
                  <a href="javascript:void(0);"
                     ...
                     data-size="xl"
                     data-title="Edit payment account — {{ $r->vouchers->first()->voucher_type ?? 'LE' }}-{{ str_pad((string) ($r->vouchers->first()->id ?? 0), 4, '0', STR_PAD_LEFT) }}"
                     data-action="{{ route('LicenseExpense.editVoucherCreditForm', $r->id) }}"
   ...
   <div class="text-center mt-5">
      <h4 class="text-muted">No License Expenses found</h4>
   </div>
   @endif
...
<script>
   ...
   function saveVisaInline(id) {
      ...
      fetch('{{ route("LicenseExpense.inlineUpdate") }}', {
         method: 'POST',
         body: payload,
         headers: {
