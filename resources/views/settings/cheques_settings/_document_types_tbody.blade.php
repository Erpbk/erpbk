  ...
  <tr data-id="{{ $doc->id }}">
    ...
    <td class="align-middle">
      @if($doc->type === 'single')
        {{ $doc->label ?: 'â€”' }}
      @else
        <span class="text-muted small">Front: {{ $doc->front_label ?? 'â€”' }}</span><br>
        <span class="text-muted small">Back: {{ $doc->back_label ?? 'â€”' }}</span>
      @endif
    </td>
    <td class="align-middle">
      @if($doc->is_active)
    ...
    <td class="text-end align-middle">
      <div class="btn-group btn-group-sm" role="group">
        <button type="button"
                ...
                data-bs-target="#editchequeDocumentTypeModal">
          <i class="ti ti-edit"></i>
        </button>
        <form method="POST"
              action="{{ route('settings-panel.cheques-settings.destroy-document-type', $doc->id) }}"
...
@empty
  <tr>
    <td colspan="6" class="text-center text-muted py-4">No document types yet. Add one to define required Cheque Documents.</td>
  </tr>
@endforelse
