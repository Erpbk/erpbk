...
class LeasingCompaniesController extends AppBaseController
{
    ...
    /** @var LeasingCompaniesRepository */
    private $leasingCompaniesRepository;

    /** @var LeasingCompanyInvoicesRepository */
    ...
    public function destroyInvoice($company_slug, $id)
    {
        ...
        try {
            ...
            CompanyQuery::table('leasing_company_invoice_items')
                ->where('inv_id', $id)
                ->delete();
        ...
        try {
            ...
            foreach ($sourceInvoice->items as $item) {
                ...
                if ($bike && $bike->status == 1) {
                    ...
                    CompanyQuery::insert('leasing_company_invoice_items', $newItemData);
                }
            }

            // Recalculate totals
            $items = CompanyQuery::table('leasing_company_invoice_items')
    ...
    public function files($company_slug, $id)
    {
        ...
        $files = CompanyQuery::table('files')->where('type', 'leasing_company')->where('type_id', $id)->latest('id')->get();
