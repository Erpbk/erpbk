...
$riderVisaBalance = company_table('legal_cases')
->where('legal_case_account_id', $accounts->id)
->sum('amount');
$riderPaidTotal = company_table('legal_cases')
->where('legal_case_account_id', $accounts->id)
->where('payment_status', 'paid')
->sum('amount');
$riderUnpaidTotal = company_table('legal_cases')
->where('legal_case_account_id', $accounts->id)
...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h3>Visa Staus #{{ $data->case_status }}</h3>
            </div>
            <!-- <div class="col-sm-6">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <table class="table table-striped">
                                <tr>
                                    <th>Visa Status</th>
                                    <td class="text-end">{{ $data->case_status }}</td>
                                </tr>
                                <tr>
                                    <th>Rider name</th>
                                    @php
    ...
    <div class="modal-dialog modal-lg modal-slide-top modal-full-top">
        <div class="modal-content">
            ...
            <div class="modal-body" id="searchTopbody">
                <form enctype="multipart/form-data" action="{{ route('LegalCase.payfine') }}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                        @include('legal_cases.voucherfield', ['data' => $data])
                        <div class="col-md-12 form-group text-center">
                            <button type="submit" class="btn btn-primary pull-right mt-3"><i class="fa fa-filter mx-2"></i> Submit</button>
                        </div>
                    </div>
