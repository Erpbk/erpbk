...
@section('title', 'Cheques Settings – Site Settings')

@push('third_party_stylesheets')
<style>
  .cheque-settings-table th {
    font-weight: 600;
    white-space: nowrap;
  }

  .cheque-settings-table .drag-handle {
    cursor: grab;
    color: #697a8d;
  }

  .cheque-settings-table .drag-handle:active {
    cursor: grabbing;
  }

  .cheque-settings-table tr.badge-soft-primary {
  ...
  .nav-tabs-cheque-fields .nav-link {
    padding: 0.5rem 0.75rem;
  }

  .cheque-top-visibility-controls {
    background: #f8f9fb;
  ...
  .cheque-top-visibility-controls .form-check {
    min-height: auto;
    margin-bottom: 0;
    display: inline-flex;
    align-items: center;
  ...
  .cheque-top-visibility-controls .form-check-input {
    width: 2rem;
    height: 1.1rem;
    margin: 0;
    cursor: pointer;
  ...
  .cheque-top-visibility-controls .form-check-label {
    font-size: 0.78rem;
    font-weight: 500;
    color: #5f6b7a;
    margin-top: 0;
  ...
  <div class="col-12">
    <div class="card mb-4">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div>
          <h4 class="card-title mb-0">Cheques Settings</h4>
          <p class="text-muted small mb-0 mt-1">
            Manage cheque categories (add, edit, reorder). Fixed Module Fields and custom fields are grouped by category; open the Module Fields tab and use each category sub-tab to manage and reorder fields.
          </p>
        </div>
      </div>
    </div>
...
{{-- Main content: tabs General | Categories | Module Fields --}}
<div class="row">
  <div class="col-12">
    <div class="card">
      <div class="card-body">
        <ul class="nav nav-tabs nav-tabs-rider-settings mb-3" id="riderSettingsMainTabs" role="tablist">
          ...
          <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-cheque-fields-btn" data-bs-toggle="tab" data-bs-target="#tab-cheque-fields" type="button" role="tab">Module Fields</button>
          </li>
          <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-cheque-documents-btn" data-bs-toggle="tab" data-bs-target="#tab-cheque-documents" type="button" role="tab">Documents</button>
          </li>
          <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-cheque-top-btn" data-bs-toggle="tab" data-bs-target="#tab-cheque-top" type="button" role="tab">Top bar</button>
          </li>
        ...
        <div class="tab-content" id="riderSettingsTabContent">
          ...
          <div class="tab-pane fade show active" id="tab-general" role="tabpanel">
            ...
            <form action="{{ route('settings-panel.cheques-settings.store-module-label') }}" method="POST" class="row g-3 align-items-end">
              @csrf
              <div class="col-md-6">
                <label class="form-label">Name in menu</label>
                <input type="text" name="module_label" class="form-control" value="{{ old('module_label', $moduleLabel ?? 'Cheques') }}" placeholder="Cheques" maxlength="100" required>
              </div>
              <div class="col-md-6">
          ...
          <div class="tab-pane fade" id="tab-cheque-documents" role="tabpanel">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
              <p class="text-muted small mb-0">Define document types required for cheques. Single = one file per type (e.g. Profile Photo). Dual = front and back page (e.g. Passport first/second). Key is used to match uploaded file names.</p>
            ...
            <div class="table-responsive">
              <table class="table table-hover cheque-settings-table mb-0">
                ...
                <tbody id="riderDocumentTypesTbody" class="rider-document-types-sortable-tbody">
                  @include('settings.cheques_settings._document_types_tbody', ['documentTypes' => $documentTypes])
                </tbody>
              </table>
            </div>
          </div>
          ...
          {{-- Tab 5: Top bar --}}
          <div class="tab-pane fade" id="tab-cheque-top" role="tabpanel">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
              <p class="text-muted small mb-0">Create a Top bar category first, then add multiple options under each category.</p>
          ...
          {{-- Tab 4: Module Fields (all-fields + category tabs) --}}
          <div class="tab-pane fade" id="tab-cheque-fields" role="tabpanel">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
            ...
            <ul class="nav nav-tabs nav-tabs-cheque-fields mb-3" id="riderFieldsCategoryTabs" role="tablist">
              <li class="nav-item" role="presentation">
                  ...
                  <table class="table table-hover cheque-settings-table mb-0">
                    <thead class="table-light">
                      <tr>
                        ...
                        <th class="text-center">Show in cheque form</th>
                    ...
                    <tbody id="rider-fields-tbody-all">
                      ...
                      <tr data-field-key="{{ $row->field_key }}" data-field-label="{{ $row->label }}" data-category-id="{{ $row->category_id ?? '' }}" data-is-visible="{{ ($row->is_visible ?? true) ? 1 : 0 }}" data-is-required="{{ ($row->is_required ?? false) ? 1 : 0 }}" data-input-type="{{ $row->input_type ?? 'text' }}" data-input-config='@json($row->input_config ?? [])' class="{{ !($row->is_visible ?? true) ? 'table-secondary' : '' }}">
                        ...
                        <td class="align-middle text-center">
                          <div class="form-check form-switch d-inline-block mb-0">
                            <input type="checkbox" class="form-check-input rider-field-required-toggle" id="req-all-{{ $row->field_key }}" data-field-key="{{ $row->field_key }}" {{ ($row->is_required ?? false) ? 'checked' : '' }} title="Require this field in cheque add/edit forms">
                            <label class="form-check-label visually-hidden" for="req-all-{{ $row->field_key }}">Mark as required</label>
                          </div>
                        </td>
                        <td class="align-middle text-center">
                          <div class="form-check form-switch d-inline-block mb-0">
                            <input type="checkbox" class="form-check-input rider-field-visibility-toggle" id="vis-all-{{ $row->field_key }}" data-field-key="{{ $row->field_key }}" {{ ($row->is_visible ?? true) ? 'checked' : '' }} title="Hide from cheque Add/Edit/View when unchecked">
                            <label class="form-check-label visually-hidden" for="vis-all-{{ $row->field_key }}">Show in cheque form</label>
                          </div>
                        </td>
                        <td class="align-middle">
                          <form action="{{ route('settings-panel.cheques-settings.update-field-assignment') }}" method="POST" class="d-flex justify-content-center rider-field-assignment-form">
                            @csrf
                      ...
                      @empty
                      <tr>
                        <td colspan="7" class="text-center text-muted py-3">No Module Fields found.</td>
                      </tr>
                      @endforelse
                      ...
                      <tr class="table-light">
                        ...
                        <td class="align-middle text-center">
                          <div class="form-check form-switch d-inline-block mb-0">
                            <input type="checkbox"
                              ...
                              data-update-url="{{ route('settings-panel.cheques-settings.update-custom-field-flags', ['id' => $customField->id]) }}"
                        ...
                        <td class="align-middle">
                          <form action="{{ route('settings-panel.cheques-settings.assign-custom-field-category', ['id' => $customField->id]) }}" method="POST" class="d-flex justify-content-center">
                            @csrf
                        ...
                        <td class="align-middle text-end">
                          <div class="btn-group btn-group-sm" role="group">
                            <button type="button" class="btn btn-sm btn-outline-primary btn-edit-rider-field"
                              ...
                              data-update-url="{{ route('settings-panel.cheques-settings.update-field', ['id' => $customField->id]) }}"
                              data-bs-toggle="modal" data-bs-target="#editRiderFieldModal">
                              <i class="ti ti-pencil"></i>
                            </button>
                            <form method="POST"
                              class="d-inline rider-destroy-field-form"
                              action="{{ route('settings-panel.cheques-settings.destroy-field', ['id' => $customField->id]) }}"
                      ...
                      <tr data-field-key="{{ $row->field_key }}" data-field-label="{{ $row->label }}" data-category-id="{{ $group->category->id }}" data-is-visible="{{ ($row->is_visible ?? true) ? 1 : 0 }}" data-is-required="{{ ($row->is_required ?? false) ? 1 : 0 }}" data-input-type="{{ $row->input_type ?? 'text' }}" data-input-config='@json($row->input_config ?? [])' class="{{ !($row->is_visible ?? true) ? 'table-secondary' : '' }}">
                        ...
                        <td class="align-middle text-center">
                          <div class="form-check form-switch d-inline-block mb-0">
                            ...
                            <label class="form-check-label visually-hidden" for="vis-{{ $group->category->id }}-{{ $row->field_key }}">Show in cheque form</label>
                          </div>
                        </td>
                        <td class="align-middle">
                          <form action="{{ route('settings-panel.cheques-settings.update-field-assignment') }}" method="POST" class="d-flex justify-content-center rider-field-assignment-form">
...
{{-- Add cheque Field modal --}}
<div class="modal fade" id="addRiderFieldModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title">Add New cheque Field</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="formAddRiderField" action="{{ route('settings-panel.cheques-settings.store-field') }}" method="POST">
        @csrf
        <div class="modal-body pt-0">
          <div class="add-rider-field-form">
            ...
            <div class="mb-3">
              <label class="form-label">Show in cheque Form</label>
              <div class="d-flex gap-3">
                <div class="form-check">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form id="formAddChequeTopCategory">
        ...
        <div class="modal-body pt-0">
          <div class="mb-3">
            <label class="form-label">cheque Dropdown Column <span class="text-danger">*</span></label>
            <select name="cheque_column" id="addChequeTopCategoryColumn" class="form-select select2" data-placeholder="Select dropdown column" required>
            ...
            <div class="form-text">Only cheque dropdown columns are available here; foreign-key and externally linked columns are excluded.</div>
          </div>
        </div>
        <div class="modal-footer border-0 pt-0">
          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
      ...
      <form id="formAddChequeTopOption">
        ...
        <div class="modal-body pt-0">
          ...
          <div class="mb-3">
            ...
            <div class="form-text">Values are loaded from the selected category column in the cheque table. You can add one or more items.</div>
          </div>
        </div>
        <div class="modal-footer border-0 pt-0">
          <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
...
{{-- Add cheque Document Type modal --}}
...
{{-- Edit cheque Document Type modal --}}
...
<script>
  (function() {
    ...
    window.refreshRiderCategoriesTable = function() {
      fetch("{{ route('settings-panel.cheques-settings.categories-table-body') }}", {
          headers: {
            'X-Requested-With': 'XMLHttpRequest'
          }
        })
    ...
    // Select2 inside Top bar modal (with correct dropdown parent).
    function initTopCategoryColumnSelect2() {
      if (!(window.jQuery && jQuery.fn && jQuery.fn.select2)) return;
      var $topCategoryColumn = jQuery('#addChequeTopCategoryColumn');
      var $modal = jQuery('#addChequeTopCategoryModal');
    ...
    if (window.jQuery) {
      jQuery('#addChequeTopCategoryModal').on('shown.bs.modal', function() {
        initTopCategoryColumnSelect2();
      });
    }
    ...
    function setChequeTopOptionSuggestions(values) {
      ChequeTopAvailableValues = Array.isArray(values) ? values : [];
      var rowsWrap = document.getElementById('addChequeTopOptionRows');
      if (!rowsWrap) return;
      rowsWrap.querySelectorAll('.cheque-top-option-row-input').forEach(function(selectEl) {
        ...
        if (currentValue && ChequeTopAvailableValues.indexOf(currentValue) !== -1) {
    ...
    if (formAddCat) {
      formAddCat.addEventListener('submit', function(e) {
        ...
        fetch(form.action || "{{ route('settings-panel.cheques-settings.store-category') }}", {
            method: 'POST',
            body: fd,
            headers: {
              'X-CSRF-TOKEN': csrf,
      ...
      formAddChequeTopCategory.addEventListener('submit', function(e) {
        ...
        fetch("{{ route('settings-panel.cheques-settings.store-cheque-top-category') }}", {
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('addChequeTopCategoryModal'));
                if (modal) modal.hide();
              }
              form.reset();
    ...
    document.addEventListener('click', function(e) {
      ...
      const fieldValuesUrlTemplate = "{{ route('settings-panel.cheques-settings.cheque-top-category-field-values', ['id' => '__CID__']) }}";
      ...
      fetch(fieldValuesUrl, {
        ...
        .then(function(data) {
          ...
          setChequeTopOptionSuggestions(values);
          if (values.length && rowsWrap) {
            var firstInput = rowsWrap.querySelector('.cheque-top-option-row-input');
    ...
    document.addEventListener('change', function(e) {
      ...
      if (!modeInput) return;
      setChequeTopSelectionMode(modeInput.value || 'single');
    });
    ...
    document.addEventListener('click', function(e) {
      ...
      var doDelete = function() {
        ...
        const deleteUrlTemplate = "{{ route('settings-panel.cheques-settings.destroy-cheque-top-category', ['id' => '__CID__']) }}";
    ...
    document.addEventListener('click', function(e) {
      ...
      var doDelete = function() {
        var fd = new FormData();
        fd.append('_method', 'DELETE');
        const deleteUrlTemplate = "{{ route('settings-panel.cheques-settings.destroy-cheque-top-option', ['id' => '__OID__']) }}";
    ...
    document.addEventListener('change', function(e) {
      ...
      const updateVisibilityUrlTemplate = "{{ route('settings-panel.cheques-settings.update-cheque-top-category-visibility', ['id' => '__CID__']) }}";
      ...
      formAddChequeTopOption.addEventListener('submit', function(e) {
        ...
        fetch("{{ route('settings-panel.cheques-settings.store-cheque-top-option') }}", {
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('addChequeTopOptionModal'));
                if (modal) modal.hide();
              }
              form.reset();
              window.refreshChequeTopAccordion();
    ...
    if (formEditChequeTopCategory) {
      formEditChequeTopCategory.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var id = document.getElementById('editChequeTopCategoryId') && document.getElementById('editChequeTopCategoryId').value;
        var btn = document.getElementById('editChequeTopCategorySubmitBtn');
        if (!id) return;
        if (btn) btn.disabled = true;

        var fd = new FormData(form);
        fd.append('_method', 'PUT');
        const updateUrlTemplate = "{{ route('settings-panel.cheques-settings.update-cheque-top-category', ['id' => '__CID__']) }}";
        ...
        fetch(updateUrl, {
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('editChequeTopCategoryModal'));
                if (modal) modal.hide();
              }
              window.refreshChequeTopAccordion();
              if (typeof Swal !== 'undefined') Swal.fire({
    ...
    if (formEditChequeTopOption) {
      formEditChequeTopOption.addEventListener('submit', function(e) {
        e.preventDefault();
        var form = this;
        var id = document.getElementById('editChequeTopOptionId') && document.getElementById('editChequeTopOptionId').value;
        var btn = document.getElementById('editChequeTopOptionSubmitBtn');
        if (!id) return;
        if (btn) btn.disabled = true;

        var fd = new FormData(form);
        fd.append('_method', 'PUT');
        const updateUrlTemplate = "{{ route('settings-panel.cheques-settings.update-cheque-top-option', ['id' => '__OID__']) }}";
        ...
        fetch(updateUrl, {
          ...
          .then(function(data) {
            ...
            if (data.success) {
              if (typeof bootstrap !== 'undefined' && bootstrap.Modal) {
                var modal = bootstrap.Modal.getInstance(document.getElementById('editChequeTopOptionModal'));
                if (modal) modal.hide();
              }
              window.refreshChequeTopAccordion();
              if (typeof Swal !== 'undefined') Swal.fire({
    ...
    document.getElementById('formEditRiderCategory').addEventListener('submit', function(e) {
      ...
      var fd = new FormData(form);
      fd.set('_method', 'PUT');
      var updateCategoryUrlTemplate = "{{ route('settings-panel.cheques-settings.update-category', ['company_slug' => request()->route('company_slug') ?? session('company_slug'), 'id' => '__ID__']) }}";
    ...
    if (formEditRiderFixedField) {
      ...
      formEditRiderFixedField.addEventListener('submit', function(e) {
        ...
        fetch("{{ route('settings-panel.cheques-settings.update-field-assignment') }}", {
            method: 'POST',
            body: assignmentFd,
            headers: {
              'X-CSRF-TOKEN': csrf,
          ...
          .then(function(data) {
            ...
            return fetch('{{ route("settings-panel.cheques-settings.update-field-assignment-visibility") }}', {
              method: 'POST',
              headers: {
    ...
    // Documents: refresh table body
    window.refreshRiderDocumentTypesTable = function() {
      fetch("{{ route('settings-panel.cheques-settings.document-types-table-body') }}", {
          headers: {
            'X-Requested-With': 'XMLHttpRequest'
          }
        })
    ...
    if (formAddRiderDocumentType) {
      formAddRiderDocumentType.addEventListener('submit', function(e) {
        ...
        if (btn) btn.disabled = true;
        fetch("{{ route('settings-panel.cheques-settings.store-document-type') }}", {
            method: 'POST',
            body: fd,
            headers: {
              'X-CSRF-TOKEN': csrf,
    ...
    if (formEditRiderDocumentType) {
      formEditRiderDocumentType.addEventListener('submit', function(e) {
        ...
        var updateDocumentUrlTemplate = "{{ route('settings-panel.cheques-settings.update-document-type', ['company_slug' => request()->route('company_slug') ?? session('company_slug'), 'id' => '__ID__']) }}";
    ...
    // Sortable for cheque document types
    function initRiderDocumentTypesSortable() {
      ...
      tbody._sortable = window.Sortable.create(tbody, {
        ...
        onEnd: function() {
          ...
          fetch("{{ route('settings-panel.cheques-settings.reorder-document-types') }}", {
            method: 'POST',
            body: fd,
            headers: {
              'X-CSRF-TOKEN': csrf,
    ...
    // Sortable for cheque categories (manual reorder)
    ...
    function initRiderCategoriesSortable() {
      ...
      riderCategoriesSortable = new Sortable(tbody, {
        ...
        onEnd: function() {
          ...
          fetch("{{ route('settings-panel.cheques-settings.reorder-categories') }}", {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrf,
          ...
          .then(function(data) {
            ...
            if (data.success) {
              ...
              if (categoryId) {
                ...
                window.location.href = "{{ route('settings-panel.cheques-settings.index') }}?tab=rider-fields";
                return;
              }
              if (typeof Swal !== 'undefined') Swal.fire({
                toast: true,
    ...
    // Inline edit fixed cheque field display name (click label to edit)
    document.addEventListener('click', function(e) {
      ...
      function saveAndRevert() {
        ...
        fetch("{{ route('settings-panel.cheques-settings.update-field-assignment-label') }}", {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'X-CSRF-TOKEN': csrf,
      ...
      document.querySelectorAll('.rider-custom-fields-sortable-tbody').forEach(function(tbody) {
        ...
        var sortable = new Sortable(tbody, {
          ...
          onEnd: function() {
            ...
            fetch("{{ route('settings-panel.cheques-settings.reorder-fields') }}", {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'X-CSRF-TOKEN': csrf,
    ...
    // Drag-and-drop reorder for Module Fields (per category tbody)
      ...
      document.querySelectorAll('.rider-fields-sortable-tbody').forEach(function(tbody) {
        ...
        var sortable = new Sortable(tbody, {
          ...
          onEnd: function() {
            ...
            fetch("{{ route('settings-panel.cheques-settings.reorder-field-assignments') }}", {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'X-CSRF-TOKEN': csrf,
    ...
    document.addEventListener('change', function(e) {
      ...
      fetch('{{ route("settings-panel.cheques-settings.update-field-assignment-visibility") }}', {
          method: 'POST',
          headers: {
    ...
    document.addEventListener('change', function(e) {
      ...
      fetch('{{ route("settings-panel.cheques-settings.update-field-assignment-required") }}', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-CSRF-TOKEN': csrf,
    ...
    document.getElementById('tab-cheque-fields-btn') && document.getElementById('tab-cheque-fields-btn').addEventListener('shown.bs.tab', function() {
    ...
    if (document.getElementById('tab-cheque-fields').classList.contains('show')) {
