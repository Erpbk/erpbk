...
<div class="mb-3">
  ...
  <div class="d-flex flex-wrap gap-2 mb-2">
    <a href="{{ route('agreements.modal', ['company_slug' => request()->route('company_slug'), 'riderId' => $rider->id, 'category' => 'rider_contract']) }}"
       class="btn btn-primary btn-sm agreement-modal-trigger"
       data-modal-title="Rider Contract">
      <i class="ti ti-file-text me-1"></i> Generate Rider Contract
    </a>
    <a href="{{ route('agreements.modal', ['company_slug' => request()->route('company_slug'), 'riderId' => $rider->id, 'category' => 'passport_agreement']) }}"
       class="btn btn-info btn-sm agreement-modal-trigger"
       data-modal-title="Passport Agreement">
      <i class="ti ti-passport me-1"></i> Generate Passport Agreement
    </a>
...
(function () {
  function initAgreementGeneratePanel() {
    ...
    select.addEventListener('change', syncTemplateId);
        ...
        fetch(emailForm.action, {
          ...
          .then(function (response) {
            return response.json().then(function (data) {
              ...
              return { ok: false, data: { success: false, message: 'Unexpected server response.' } };
            });
          })
