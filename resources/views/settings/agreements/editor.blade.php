...
<style>
  ...
  .agreement-style-card:hover { border-color: #b4b9ca; }
  ...
  <div class="col-lg-8">
    <div class="card mb-3">
      <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
          <a href="{{ route('settings-panel.agreements.templates', ['company_slug' => request()->route('company_slug'), 'category' => $category->id]) }}" class="text-muted small">
      ...
      <div class="card-body">
        <form method="POST"
              action="{{ $template->exists
                ? route('settings-panel.agreements.update', ['company_slug' => request()->route('company_slug'), 'id' => $template->id])
                : route('settings-panel.agreements.store', ['company_slug' => request()->route('company_slug'), 'category' => $category->id]) }}"
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.agreement-style-card').forEach(function (card) {
    card.addEventListener('click', function () {
      document.querySelectorAll('.agreement-style-card').forEach(function (c) { c.classList.remove('active'); });
