@forelse($customFields as $index => $field)
  <tr data-id="{{ $field->id }}">
    ...
    <td class="align-middle rider-custom-field-index">{{ $index + 1 }}</td>
    <td class="align-middle">
      {{ $field->label }}
      @if($field->help_text)
    ...
    <td class="align-middle text-center">
      <div class="form-check form-switch d-inline-block mb-0">
        <input type="checkbox"
               ...
               data-id="{{ $field->id }}"
               data-update-url="{{ route('settings-panel.cheques-settings.update-custom-field-flags', $field->id) }}"
    ...
    <td class="text-end align-middle">
      <div class="btn-group btn-group-sm" role="group">
        <button type="button"
                ...
                data-update-url="{{ route('settings-panel.cheques-settings.update-field', $field->id) }}"
                data-bs-toggle="modal"
                data-bs-target="#editchequeFieldModal"
                onclick="
                  var f = document.getElementById('formEditchequeField');
                  if (f && this.dataset.updateUrl) f.action = this.dataset.updateUrl;
                  document.getElementById('editchequeFieldId').value = this.dataset.id;
                  document.getElementById('editchequeFieldLabel').value = this.dataset.label;
                  document.getElementById('editchequeFieldCategory').value = this.dataset.categoryId || '';
                  var prevCat = document.getElementById('editchequeFieldPreviousCategoryId');
                  if (prevCat) prevCat.value = this.dataset.categoryId || '';
                  document.getElementById('editchequeFieldDataType').value = this.dataset.dataType;
                  document.getElementById('editchequeFieldHelpText').value = this.dataset.helpText || '';
                  document.getElementById('editchequeFieldDefaultValue').value = this.dataset.defaultValue || '';
                  document.getElementById('editchequeFieldInputFormat').value = this.dataset.inputFormat || '';
                  var privacy = {};
                  try { privacy = JSON.parse(this.dataset.dataPrivacy || '{}'); } catch(e) { privacy = {}; }
                  document.getElementById('editchequeFieldPii').checked = !!privacy.pii;
                  document.getElementById('editchequeFieldEphi').checked = !!privacy.ephi;
                  ...
                  var cfgInput = document.getElementById('editchequeFieldConfigJson');
                  if (cfgInput) cfgInput.value = this.dataset.config || '{}';
                  if (typeof document.getElementById('editchequeFieldDataType').dispatchEvent === 'function') {
                    document.getElementById('editchequeFieldDataType').dispatchEvent(new Event('change'));
                  }
                ">
        ...
        <form method="POST"
              ...
              action="{{ route('settings-panel.cheques-settings.destroy-field', $field->id) }}"
              data-category-id="{{ $field->category_id }}">
          @csrf
          @method('DELETE')
          <button type="submit" class="btn btn-outline-danger btn-icon btn-destroy-rider-field">
...
@empty
  <tr>
    <td colspan="7" class="text-center text-muted py-4">
      No custom fields in this category.
    </td>
  </tr>
@endforelse
