@php
$legalCaseStatuses = company_table('legal_case_statuses')->where('is_active', 1)->orderBy('display_order', 'asc')->get();
@endphp
...
<div class="form-group col-sm-6">
    ...
    <select class="form select select2" id="case_status" name="case_status" readonly>
        <option value=""></option>
        @foreach ($legalCaseStatuses as $status)
        <option value="{{ $status->name }}" @if($data->case_status == $status->name) selected @endif>{{ $status->name }}</option>
        @endforeach
    </select>
</div>
<div class="form-group col-sm-6">
