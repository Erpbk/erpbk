...
<div class="{{ $colClass }} form-group" id="{{ $wrapperId }}" data-assign-field="number">
    <label for="sim_assign_number">{{ $label }}:</label>
    {!! Form::text('number', old('number', $simsModel->number ?? ''), ['class' => 'form-control', 'readonly' => true, 'id' => 'sim_assign_number']) !!}
</div>
@elseif($fieldKey === 'assign_to_display')
<div class="{{ $colClass }} form-group" id="{{ $wrapperId }}" data-assign-field="assign_to_display">
    <label for="sim_assign_to_display">{{ $label }}:</label>
    {!! Form::text('assign_to_display', $assignee_name ?? 'N/A', ['class' => 'form-control', 'readonly' => true, 'id' => 'sim_assign_to_display']) !!}
</div>
...
@if(in_array('rider', $assignTargets, true))
<div class="{{ $colClass }} form-group assignee-field assignee-field-rider{{ $defaultAssigneeType === 'employee' ? ' hidden-field' : '' }}" id="{{ $wrapperId }}" data-assign-field="assign_to_rider" @if($defaultAssigneeType === 'employee') hidden aria-hidden="true" @endif>
    {!! Form::label('assign_to_rider', $assignToLabel . ':', ['id' => 'label_assign_to_rider']) !!}
...
@if(in_array('employee', $assignTargets, true))
<div class="{{ $colClass }} form-group assignee-field assignee-field-employee{{ $defaultAssigneeType === 'rider' ? ' hidden-field' : '' }}" id="{{ $wrapperId }}" data-assign-field="assign_to_employee" @if($defaultAssigneeType === 'rider') hidden aria-hidden="true" @endif>
    {!! Form::label('assign_to_employee', $assignToLabel . ':', ['id' => 'label_assign_to_employee']) !!}
