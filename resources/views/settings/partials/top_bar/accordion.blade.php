...
<div class="accordion" id="riderTopAccordion">
  @forelse(($topBarCategories ?? $riderTopCategories ?? collect()) as $category)
  <div class="accordion-item">
    <h2 class="accordion-header" id="riderTopHeading{{ $category->id }}">
      <div class="d-flex align-items-center gap-2 p-2">
        ...
        <div class="d-flex align-items-center gap-2 pe-2 module-top-visibility-controls" data-category-id="{{ $category->id }}">
          <div class="form-check form-switch mb-0">
            <input class="form-check-input module-top-visibility-toggle" type="checkbox" id="riderTopBar{{ $category->id }}" data-field="show_in_top_bar" {{ ($category->show_in_top_bar ?? true) ? 'checked' : '' }}>
            <label class="form-check-label text-nowrap" for="riderTopBar{{ $category->id }}">Top Bar</label>
          </div>
          <div class="form-check form-switch mb-0">
            <input class="form-check-input module-top-visibility-toggle" type="checkbox" id="riderTopView{{ $category->id }}" data-field="show_in_view_cards" {{ ($category->show_in_view_cards ?? false) ? 'checked' : '' }}>
  ...
  @empty
  <div class="text-muted small">{{ $topBarEmptyMessage ?? 'No categories yet. Add your first category to begin.' }}</div>
  @endforelse
</div>
