...
<div class="row mb-4">
    ...
    <div class="col-md-6">
        ...
        <select class="form-select select2" id="city" name="city" data-selected="{{ old('city') ?? (isset($branch) ? ($branch->city ?? '') : '') }}">
...
<script>
    $(document).ready(function() {
        ...
        // Load emirates/cities based on company country (from settings company profile)
        const companyCountry = "{{ addslashes($companyCountry ?? '') }}";
        const emirateSelect = $('#city');
        const selectedEmirate = emirateSelect.data('selected') || '';

        function getMainCities(countryName, cities) {
            const normalized = (countryName || '').trim().toLowerCase();

            // UAE: show Emirates only
            ...
            return uniqueCities.sort((a, b) => a.localeCompare(b)).slice(0, 20);
        }
        ...
        async function initEmirateOptions() {
            const allCities = await fetchCitiesByCountry(companyCountry);
            const mainCities = getMainCities(companyCountry, allCities);

            emirateSelect.empty().append('<option value="">Select City</option>');

            mainCities.forEach(function(city) {
                const option = new Option(city, city, false, city === selectedEmirate);
                emirateSelect.append(option);
            });

            emirateSelect.trigger('change.select2');
        }
