<table class="table table-striped dataTable no-footer" id="dataTableBuilder">
    ...
    <tbody>
        ...
        <tr class="text-center">
            ...
            <td>
                <a href="javascript:void(0);" data-action="{{ route(($trashRoute ?? 'settings-panel.trash') . '.show', ['module' => 'vouchers', 'id' => $voucher->id]) }}" class="text-primary show-modal" data-title="Voucher Details - {{ $voucherId }} (Deleted)" data-size="xl">{{ $voucherId }}</a>
            </td>
            ...
            <nav>
                <ul class="pagination justify-content-center mb-0">
                    <li class="page-item {{ $currentPage == 1 ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ route(($trashRoute ?? 'settings-panel.trash') . '.index', array_merge(request()->except('page'), ['page' => $currentPage - 1])) }}">
                            Previous
                        </a>
                    </li>
                    ...
                    @for($i = 1; $i <= $totalPages; $i++)
                        ...
                        <a class="page-link" href="{{ route(($trashRoute ?? 'settings-panel.trash') . '.index', array_merge(request()->except('page'), ['page' => $i])) }}">
                            {{ $i }}
                        </a>
                        </li>
                        @endfor
