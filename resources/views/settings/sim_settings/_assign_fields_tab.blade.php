...
$settingsRoutePrefix = $assignSettingsRoutePrefix ?? 'settings-panel.sim-settings';
$settingsRouteParams = $assignSettingsRouteParams ?? ['company_slug' => request()->route('company_slug'), 'module' => 'sims'];
$assignBuiltinFields = $assignFields->where('kind', '!=', 'custom');
$assignCustomFields = $assignFields->where('kind', 'custom');
@endphp

<div class="tab-pane fade {{ $showAssignFieldsTab ? 'show active' : '' }}" id="tab-sim-assign-fields" role="tabpanel">
  <div class="d-flex justify-content-end align-items-center mb-3">
    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addSimAssignFieldModal">
  ...
  <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
    <p class="text-muted small mb-0">
      Built-in fields for SIM assign and return modals (number, user type, rider/employee, dates, notes).
      Custom fields here appear only on assign/return modals—not under <b>SIM Fields</b>.
    </p>
  </div>

  <div class="modal fade" id="addSimAssignFieldModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        ...
        <form id="formAddSimAssignField" action="{{ route($settingsRoutePrefix . '.store-assign-field', $settingsRouteParams) }}" method="POST">
          @csrf
          <div class="modal-body pt-0">
            <div class="row g-3 align-items-end">
              ...
              <div class="col-md-3">
                ...
                <select name="data_type" id="addSimAssignFieldDataType" class="form-select" required>
              ...
              <div class="col-md-6" id="addSimAssignFieldOptionsWrap">
                <label class="form-label">Dropdown Options (one per line)</label>
                <input type="hidden" name="config_options" id="addSimAssignFieldConfigOptionsHidden" value="">
                <div id="addSimAssignFieldOptionsRows" class="d-flex flex-column gap-2"></div>
                <button type="button" class="btn btn-sm btn-outline-primary mt-2" id="addSimAssignFieldOptionRowBtn">Add Option</button>
              </div>
            </div>
          </div>
  ...
  @include('settings.sim_settings._assign_field_edit_modal')

  <div class="table-responsive">
    <table class="table table-hover sim-settings-table mb-0">
      ...
      <tbody class="sim-assign-fields-builtin-sortable-tbody" id="simAssignFieldsBuiltinSortable">
        ...
        <tr data-assign-field-id="{{ $row->id }}">
          ...
          <td class="align-middle">
            <div class="d-flex flex-wrap gap-3 mt-1">
              <div class="form-check form-switch mb-0">
                <input type="checkbox" class="form-check-input Sim-assign-modal-toggle" data-id="{{ $row->id }}" data-prop="show_on_active" {{ $row->show_on_active ? 'checked' : '' }}>
                <label class="form-check-label small">Assign</label>
              </div>
              <div class="form-check form-switch mb-0">
                <input type="checkbox" class="form-check-input Sim-assign-modal-toggle" data-id="{{ $row->id }}" data-prop="show_on_change" {{ $row->show_on_change ? 'checked' : '' }}>
          ...
          <td class="align-middle text-end">
            <button type="button"
              ...
              data-bs-target="#editSimAssignFieldModal"
      ...
      <tbody class="sim-assign-fields-custom-sortable-tbody" id="simAssignFieldsCustomSortable">
...
<script>
  (function() {
    ...
    document.addEventListener('change', function(e) {
      var toggle = e.target.closest('.Sim-assign-modal-toggle');
      if (!toggle) return;
    ...
    function initAssignFieldSortable(tbodyId) {
      ...
      new Sortable(tbody, {
        ...
        animation: 150,
        onEnd: function() {
          var builtin = document.getElementById('simAssignFieldsBuiltinSortable');
          var custom = document.getElementById('simAssignFieldsCustomSortable');
          var order = [];
    ...
    function initAll() {
      initAssignFieldSortable('simAssignFieldsBuiltinSortable');
      initAssignFieldSortable('simAssignFieldsCustomSortable');
    }

    document.addEventListener('DOMContentLoaded', initAll);
    var assignTabBtn = document.getElementById('tab-sim-assign-fields-btn');
    if (assignTabBtn) {
